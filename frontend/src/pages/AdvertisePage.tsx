...
const journeyItems = [
  ...
  },
  {
    ...
    outcome:
      "آگهی بیرونی با نشانی آگهی اصلی، پس از تأیید نشانی و پروفایل منبع توسط اپراتور.",
    icon: Globe2,
  },
] as const;
...
const faqs = [
  ...
  {
    question: "استخراج اطلاعات منبع چطور انجام می‌شود؟",
    answer:
      "پس از تأیید نشانی، کشف منبع انجام می‌شود. اپراتور پروفایل منبع را بررسی و حالت نیازمند تأیید یا خودکار را انتخاب می‌کند. در حالت نیازمند تأیید، نتایج معتبر یکجا بررسی می‌شوند؛ در حالت خودکار منتشر می‌شوند. موارد استثنا همیشه بررسی جداگانه دارند.",
  },
  {
      ...
      >
        <div className="border-primary/25 bg-primary/5 flex gap-4 rounded-2xl border p-5 sm:p-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-2 leading-7">
              پس از تأیید پروفایل، نماینده منبع می‌تواند نشانی‌های همان میزبان
              را برای استخراج بفرستد و نتیجه اجراها را در داشبورد پیگیری کند.
              تصاویر از منبع مجاز دریافت و پردازش می‌شوند؛ خرابی تصویر مانع
              انتشار اطلاعات معتبر نیست. لغو تخصیص منبع، درخواست‌های تازه و
              انتشار را متوقف و آگهی‌های آن تخصیص را غیرفعال می‌کند.
            </p>
          </div>
        </div>
      </section>
