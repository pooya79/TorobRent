...
export async function changeSourcePublicationMode(
  proposalId: string,
  body: components["schemas"]["SourcePublicationModeRequest"],
) {
  const { data, error } = await api.POST(
...
export async function changeSourceProcessing(
  proposalId: string,
  body: components["schemas"]["SourceProcessingRequest"],
) {
  const { data, error } = await api.POST(
