...
from .models import City, District, Listing, Neighborhood, Property
...
def _with_normalized_name(queryset: QuerySet[Any, Any]) -> QuerySet[Any, Any]:
    expression = Replace("name_fa", Value("ي"), Value("ی"), output_field=CharField())
    expression = Replace(expression, Value("ى"), Value("ی"))
    expression = Replace(expression, Value("ك"), Value("ک"))
    expression = Replace(expression, Value("آ"), Value("ا"))
    expression = Replace(expression, Value("أ"), Value("ا"))
    expression = Replace(expression, Value("إ"), Value("ا"))
...
def autocomplete_locations(query: str, *, limit: int = 10) -> list[LocationSuggestion]:
    ...
    cities = _with_normalized_name(City.objects.filter(reviewed=True)).filter(
    ...
    districts = _with_normalized_name(
        District.objects.filter(reviewed=True, city__reviewed=True).select_related("city")
...
def search_properties(location: str = "") -> QuerySet[Property]:
    ...
    properties = Property.objects.select_related("city", "district", "neighborhood").annotate(
    ...
    if location:
        try:
            ...
            city_ids = _with_normalized_name(City.objects.all()).filter(
                search_name__icontains=normalized_location
            )
            district_ids = _with_normalized_name(District.objects.all()).filter(
                search_name__icontains=normalized_location
            )
            neighborhood_ids = _with_normalized_name(Neighborhood.objects.all()).filter(
                search_name__icontains=normalized_location
            )
