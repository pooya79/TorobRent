...
FOCUSED_MEASUREMENT_DEDUP_SECONDS = 5
...
def _dispatch_focused_measurement(property_id: uuid.UUID) -> None:
    ...
    key = f"catalog:property-match-focused:{property_id}"
    if not cache.add(key, True, timeout=FOCUSED_MEASUREMENT_DEDUP_SECONDS):
        return
    try:
