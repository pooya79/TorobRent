...
def _run_property_match_operation(
    *,
    ...
    ensure_property_match_operation(operation_id=parsed_id, kind=kind)
    try:
        result, advanced = process_property_match_operation_page(
        ...
        record_property_match_operation_failure(operation_id=parsed_id, kind=kind, error=error)
        raise
    ...
    if should_schedule and (advanced or result["generation"] > generation):
        ...
        def dispatch_next_page() -> None:
            try:
                ...
                record_property_match_operation_failure(
                    operation_id=parsed_id, kind=kind, error=error
                )
                raise

        transaction.on_commit(dispatch_next_page)
    return result
