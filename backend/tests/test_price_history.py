...
def test_price_history_records_only_changed_published_pairs_and_serializes_toman():
    ...
    initial_count = listing.price_history.count()
    assert initial_count >= 3
    listing.available_until = timezone.now() + timedelta(days=30)
    listing.save()
    assert listing.price_history.count() == initial_count
    original = listing.price_history.order_by("recorded_at", "id").first()
    ...
    listing.terms.save()
    assert listing.price_history.count() == initial_count + 1
    ...
    assert (
        result["price_history"][-1]["monthly_rent_toman"] == listing.terms.monthly_rent_rial // 10
    )
    ...
    listing.save()
    assert listing.price_history.count() == initial_count + 2
